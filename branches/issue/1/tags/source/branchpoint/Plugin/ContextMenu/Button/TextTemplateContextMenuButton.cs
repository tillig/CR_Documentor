...
namespace CR_Documentor.ContextMenu.Button
{
	...
	public class TextTemplateContextMenuButton : ContextMenuButton
	{
		...
		#region TextTemplateContextMenuButton Variables
		...
		/// <summary>
		/// Internal storage for the <see cref="Content"/> property.
		/// </summary>
		private string _Content = "";

		/// <summary>
		/// Internal storage for the <see cref="ConvertSelectionToLower"/> property.
		/// </summary>
		private bool _ConvertSelectionToLower = false;

		/// <summary>
		/// Internal storage for the <see cref="SelectionRequirements"/> property.
		/// </summary>
		private DXCoreContext.SelectionContext _SelectionRequirements = DXCoreContext.SelectionContext.None;
		...
		#region TextTemplateContextMenuButton Properties
		...
		/// </value>
		public virtual bool ConvertSelectionToLower
		{
			get
			{
				return _ConvertSelectionToLower;
			}
			set
			{
				_ConvertSelectionToLower = value;
			}
		}
		...
		/// </value>
		public virtual DXCoreContext.SelectionContext SelectionRequirements
		{
			get
			{
				return _SelectionRequirements;
			}
			set
			{
				_SelectionRequirements = value;
			}
		}
		...
		#region TextTemplateContextMenuButton Implementation
		...
		{
			Log.Enter(ImageType.Info, "Executing text insertion/replacement.");
			...
			try
			{
				...
				if (CodeRush.Selection.Exists)
				{
					...
					Log.Send("Selection exists.  Saving selected text and active range.");
				...
				if (replacement != "")
				{
					Log.Send("Formatting replacement text in preparation for embedding.");
				...
				{
					Log.Send("Removing double-newlines at end of replacement.");
				...
				Log.Send("Processing beginning of insertion line.");
				...
				Log.Send("Processing end of insertion line.");
				...
				Log.Send("Adding in XML comment prefixes.");
				fullContent = fullContent.Replace("\n", "\n" + DocumentorContextMenu.CommentPrefix + " ");

				Log.Send("Converting newlines back to system value.");
				...
				Log.Send("Removing any existing selection in preparation for insert/embed.");
				CodeRush.Selection.Delete();

				Log.Send("Inserting/embedding text.");
				...
				Log.Send("Formatting final insertion.");
				...
				Log.Send("Insert/embed complete.");
			}
			catch (Exception err)
			{
